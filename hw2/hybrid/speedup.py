...
base_communication_time = communication_times[0]
...
communication_speedup = [base_communication_time / time for time in communication_times]
...
plt.plot(process_counts, computation_speedup, marker='o', linestyle='-', color='#F4CE5D', label='Computation time speedup')
plt.plot(process_counts, communication_speedup, marker='o', linestyle='-', color='#B35751', label='Communication time speedup')
plt.plot(process_counts, ideal_speedup, marker='o', linestyle='-', color='green', label='ideal time speedup')

# 設定圖表標籤與標題
plt.xlabel('number of proocesses', fontsize=14)
plt.ylabel('Speedup', fontsize=14)
# plt.title('Speedup - Pthread, #strict_34', fontsize=16)
# plt.title('Speedup - Pthread, #slow_01', fontsize=16)
# plt.title('Speedup - Pthread, #slow_01', fontsize=16)
plt.title('Speedup - Hybrid, Node = 1, 2 cores/process, slave, #strict_34', fontsize=16)
# plt.title('Speedup - Hybrid, Node = 1, 2 cores/process, master, #strict_34', fontsize=16)

# 顯示圖例
plt.legend()

# 儲存圖檔
# output_image = 'speedup_master.png'
output_image = 'speedup_slave.png'
