...
__global__ void sobel (unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    ...
    /* Hint 6 */
    // parallel job by blockIdx, blockDim(每個block中有多少thread), threadIdx 
...
int main(int argc, char** argv) {
    ...
    dim3 grid(ceil(width / float(threadsPerBlock.x)), ceil(height / float(threadsPerBlock.y)));
    sobel<<<grid, threadsPerBlock>>>(d_s, d_t, height, width, channels);
